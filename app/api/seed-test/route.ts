...
// Rota de uso único para criar o post de teste
// Acesse: /api/seed-test?secret=achadinhos2024

export async function GET(req: NextRequest) {
  const secret = req.nextUrl.searchParams.get("secret");
  if (secret !== process.env.ADMIN_SECRET && secret !== "achadinhos2024") {
  ...
  try {
    ...
    const produto1 = await prisma.product.upsert({
      where: { slug: "cortina-solar-blackout-varanda-ml" },
      update: {},
      create: {
        slug: "cortina-solar-blackout-varanda-ml",
        ...
        urlMercadoLivre: "https://www.mercadolivre.com.br/p/MLBU3721770783",
        prosDefault:
          "Bloqueia 99% da luz solar\nReduz calor e protege móveis do sol\nFácil instalação\nBom custo-benefício\nDisponível em várias cores",
        contrasDefault: "Lavagem apenas à mão\nNão acompanha trilho",
      },
    });

    // Produto 2 — Cortina Oxford Blackout (Amazon)
    const produto2 = await prisma.product.upsert({
      where: { slug: "cortina-oxford-blackout-varanda-amazon" },
      update: {},
      create: {
        slug: "cortina-oxford-blackout-varanda-amazon",
        ...
        contrasDefault: "Preço um pouco acima da média\nOpções de cor limitadas",
      },
    });
    ...
    const conteudoJson = JSON.stringify({
      ...
      criterios: [
        ...
        },
        {
          ...
          descricao:
            "Analisamos a qualidade do tecido, resistência ao desbotamento e capacidade de aguentar exposição constante ao sol sem perder a forma.",
        },
        {
      ...
      secoes_complementares: [
        {
          ...
          conteudo: [
            "O primeiro passo é medir o espaço corretamente. Cortinas muito curtas ficam estranhas visualmente e não cumprem a função de proteção. O ideal é que a cortina tenha ao menos 10 cm a mais que a altura do vão, e largura suficiente para cobrir bem quando fechada.",
    ...
    const post = await prisma.post.upsert({
      ...
      create: {
        ...
        titulo:
          "Melhor Cortina de Varanda Contra Sol: Comparamos as Top 2 do Mercado",
        resumo:
        ...
        metaTitle:
          "Melhor Cortina de Varanda Contra Sol 2024 – Achadinhos da Elis",
        metaDescription:
    ...
    await prisma.postProduct.createMany({
      data: [
        ...
        },
        {
          ...
          indicadoPara:
            "Quem precisa de maior durabilidade e tem varanda mais exposta.",
        },
      ],
    });
    ...
    return NextResponse.json({
      ...
      url: `/reviews/cortina-varanda-contra-sol-melhor-opcao`,
    });
  } catch (e) {
    return NextResponse.json(
